...
// reades the content of a file (size bytes), if shouldMatchSize is false will
// read the entire fill untill size bytes are read or the entire file was read
// can return error: couldNotOpenFinle, fileSizeDitNotMatch
Errors readEntireFile(void *data, size_t size, const char *name,
                      bool shouldMatchSize, int *bytesRead = nullptr);
...
// saved the data with a check sum. It will use a temporary file to make sure
// the saving is safe. can return error: couldNotOpenFinle
...
// loads the data that was saved using safeSave and stored as a
// SafeSafeKeyValueData structure can return error: couldNotOpenFinle,
// checkSumFailed, fileSizeNotBigEnough 	readBackup (if
// reportLoadingBackupAsAnError but data will still be loaded with the backup)
...
struct SafeSafeKeyValueData {
  ...
  SafeSafeKeyValueData(
      std::initializer_list<std::pair<std::string, Entry>> init) {
...
// saved the data stored as a SafeSafeKeyValueData structure in a binary format
// with a check sum and a backup Uses a temporary file to make sure the saving
// is safe can return error: couldNotOpenFinle, 	couldNotMakeBackup (if
// reportnotMakingBackupAsAnError is true, but will still save the first file)
Errors safeSave(SafeSafeKeyValueData &data, const char *nameWithoutExtension,
                bool reportnotMakingBackupAsAnError);

// saved the data stored as a SafeSafeKeyValueData structure in a binary format
// with a check. Uses a temporary file to make sure the saving is safe can
// return error: couldNotOpenFinle,
Errors safeSaveNoBackup(SafeSafeKeyValueData &data,
                        const char *nameWithoutExtension);

// loads the data that was saved using safeSave and stored as a
// SafeSafeKeyValueData structure can return error: couldNotOpenFinle,
// fileSizeNotBigEnough, checkSumFailed, couldNotParseData 	readBackup (if
// reportLoadingBackupAsAnError but data will still be loaded from the backup)
