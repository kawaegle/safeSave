...
//
//  safeSave 1.0.4
//  added the ability to easily store a key value pare inside itself, easy
//  nesting data!
...
Errors readEntireFile(void *data, size_t size, const char *name,
                      bool shouldMatchSize, int *bytesRead) {
  if (bytesRead) {
    *bytesRead = 0;
  }
...
Errors writeEntireFileWithCheckSum(const void *data, size_t size,
                                   const char *name) {
...
Errors safeSave(const void *data, size_t size, const char *nameWithoutExtension,
  ...
  if (err == couldNotOpenFinle) {
    ...
    std::filesystem::copy_file(
        file1, file2, std::filesystem::copy_options::overwrite_existing,
        errorCode);
    // auto err2 = writeEntireFileWithCheckSum((char*)data, size,
    // file2.c_str());
...
Errors safeSaveNoBackup(const void *data, size_t size,
...
Errors safeLoadBackup(void *data, size_t size,
                      const char *nameWithoutExtension) {
  std::string file2 = nameWithoutExtension;
  file2 += "2.bin";
...
Errors safeSave(SafeSafeKeyValueData &data, const char *nameWithoutExtension,
                bool reportnotMakingBackupAsAnError) {
  auto rez = data.formatIntoFileDataBinary();

  return safeSave(rez.data(), rez.size(), nameWithoutExtension,
                  reportnotMakingBackupAsAnError);
}

Errors safeSaveNoBackup(SafeSafeKeyValueData &data,
                        const char *nameWithoutExtension) {
  auto rez = data.formatIntoFileDataBinary();
...
Errors safeLoad(SafeSafeKeyValueData &data, const char *nameWithoutExtension,
                bool reportLoadingBackupAsAnError) {
  data = {};

  std::vector<char> readData;
  auto errCode =
      safeLoad(readData, nameWithoutExtension, reportLoadingBackupAsAnError);
...
Errors openFileMapping(FileMapping &fileMapping, const char *name, size_t size,
  ...
  fileMapping.internal.fileHandle =
      CreateFileA(name, GENERIC_READ | GENERIC_WRITE, 0, NULL,
                  createDisposition, FILE_ATTRIBUTE_NORMAL, NULL);
  ...
  fileMapping.internal.fileMapping = CreateFileMappingA(
      fileMapping.internal.fileHandle, NULL, PAGE_READWRITE, 0, size, NULL);
  ...
  fileMapping.pointer = MapViewOfFile(fileMapping.internal.fileMapping,
...
bool SafeSafeKeyValueData::operator==(const SafeSafeKeyValueData &other) const {
  ...
  for (const auto &e : entries) {
    ...
    if (found == other.entries.end()) {
      return false;
    }

    if (e.second != found->second) {
      return false;
    }
  }
...
Errors SafeSafeKeyValueData::setRawData(std::string at, void *data,
                                        size_t size) {
  auto it = entries.find(at);
...
Errors SafeSafeKeyValueData::setVec3(std::string at, float x, float y,
                                     float z) {
  auto it = entries.find(at);
...
Errors SafeSafeKeyValueData::setVec4(std::string at, float x, float y, float z,
                                     float w) {
  auto it = entries.find(at);
...
Errors SafeSafeKeyValueData::setIVec4(std::string at, int x, int y, int z,
                                      int w) {
  auto it = entries.find(at);
...
Errors SafeSafeKeyValueData::getVec3(std::string at, float &x, float &y,
                                     float &z) {
  auto it = entries.find(at);
...
Errors SafeSafeKeyValueData::getVec4(std::string at, float &x, float &y,
                                     float &z, float &w) {
  auto it = entries.find(at);
...
Errors SafeSafeKeyValueData::getIVec4(std::string at, int &x, int &y, int &z,
                                      int &w) {
  auto it = entries.find(at);
...
Errors SafeSafeKeyValueData::setKeyValueData(std::string at,
                                             const SafeSafeKeyValueData &data) {
...
Errors SafeSafeKeyValueData::getKeyValueData(std::string at,
                                             SafeSafeKeyValueData &data) {
  auto it = entries.find(at);
...
Errors SafeSafeKeyValueData::getRawDataPointer(std::string at, void *&data,
                                               size_t &size) {
  data = 0;
  size = 0;
...
std::vector<char> sfs::SafeSafeKeyValueData::formatIntoFileDataBinary() const {
  ...
  for (auto &e : entries) {
    ...
    if (d.type == Entry::Types::rawData_type ||
        d.type == Entry::Types::string_type ||
...
bool SafeSafeKeyValueData::Entry::operator==(const Entry &other) const {
  ...
  case int_type: {
    if (primitives.intData != other.primitives.intData) {
      return false;
    }
    break;
  }
  case float_type: {
    if (primitives.floatData != other.primitives.floatData) {
      return false;
    }
    break;
  }
  case bool_type: {
    if (primitives.boolData != other.primitives.boolData) {
      return false;
    }
    break;
  }
  ...
  case uint64_type: {
    if (primitives.uint64Data != other.primitives.uint64Data) {
      return false;
    }
    break;
  }
  case int64_type: {
    if (primitives.int64Data != other.primitives.int64Data) {
      return false;
    }
    break;
  }
  case keyValueData_type: {
    if (data != other.data) {
      return false;
    }
    break;
  ...
  case char_type: {
    if (primitives.charData != other.primitives.charData) {
      return false;
    }
    break;
  }
  case uchar_type: {
    if (primitives.uCharData != other.primitives.uCharData) {
      return false;
    }
    break;
  }
  case uint_type: {
    if (primitives.uintData != other.primitives.uintData) {
      return false;
    }
    break;
  }
  case double_type: {
    if (primitives.doubleData != other.primitives.doubleData) {
      return false;
    }
    break;
  }
  ...
  case vec2_type: {
    if (primitives.vec.x != primitives.vec.x ||
        primitives.vec.y != primitives.vec.y) {
      return false;
    }
    break;
  }
  case vec3_type: {
    if (primitives.vec.x != primitives.vec.x ||
        primitives.vec.y != primitives.vec.y ||
        primitives.vec.z != primitives.vec.z) {
      return false;
    }
    break;
  }
  case vec4_type: {
    if (primitives.vec.x != primitives.vec.x ||
        primitives.vec.y != primitives.vec.y ||
        primitives.vec.z != primitives.vec.z ||
        primitives.vec.w != primitives.vec.w) {
      return false;
    }
    break;
  }
  ...
  case ivec2_type: {
    if (primitives.ivec.x != other.primitives.ivec.x ||
        primitives.ivec.y != other.primitives.ivec.y) {
      return false;
    }
    break;
  }
  case ivec3_type: {
    if (primitives.ivec.x != other.primitives.ivec.x ||
        primitives.ivec.y != other.primitives.ivec.y ||
        primitives.ivec.z != other.primitives.ivec.z) {
      return false;
    }
    break;
  }
  case ivec4_type: {
    if (primitives.ivec.x != other.primitives.ivec.x ||
        primitives.ivec.y != other.primitives.ivec.y ||
        primitives.ivec.z != other.primitives.ivec.z ||
        primitives.ivec.w != other.primitives.ivec.w) {
      return false;
    }
    break;
  }
